...
public class CustomBoatRenderer extends BoatRenderer {
    ...
    public CustomBoatRenderer(EntityRendererProvider.Context pContext, boolean pChestBoat) {
        ...
        this.boatResources = Stream.of(CustomBoat.Type.values()).collect(ImmutableMap.toImmutableMap(type -> type,
                type -> Pair.of(new ResourceLocation(CreateAllTheFoods.MOD_ID, getTextureLocation(type, pChestBoat)), this.createBoatModel(pContext, type, pChestBoat))));
    }

    private static String getTextureLocation(CustomBoat.Type pType, boolean pChestBoat) {
    ...
    private static ModelLayerLocation createLocation(String pPath, String pModel) {
        return new ModelLayerLocation(new ResourceLocation(CreateAllTheFoods.MOD_ID, pPath), pModel);
    }

    public Pair<ResourceLocation, ListModel<Boat>> getModelWithLocation(Boat boat) {
        if(boat instanceof CustomBoat customBoat) {
            return this.boatResources.get(customBoat.getModVariant());
        } else if(boat instanceof CustomChestBoat customChestBoat) {
            return this.boatResources.get(customChestBoat.getModVariant());
        } else {
            return null;
        }
    }
